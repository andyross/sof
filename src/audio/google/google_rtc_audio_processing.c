...
static enum sof_ipc_frame bits_fmt(int bits)
{
	if (bits == 32)
					       ...
					       int num_of_sinks)
{
	...
	if (ref_fmt != bits_fmt(CONFIG_GOOGLE_RTC_AUDIO_PROCESSING_REF_BITS)) {
		comp_err(dev, "Reference stream must be %d bit samples\n",
