 ...
 */
#define TEST_SGEN_ID MT8196_MEMIF_DL1
...
enum {
	MT8196_SGEN_UL0 = 0x18,
	MT8196_SGEN_UL1 = 0x1f,
	MT8196_SGEN_UL2 = 0x2f,
	MT8196_SGEN_DL1 = 0x47,
	MT8196_SGEN_DL_24CH = 0x60,
};
...
static uint32_t mt8196_sinegen_timing(uint32_t rate)
{
...
static void mt8196_afe_sinegen_enable(uint32_t sgen_id, uint32_t rate, int enable)
{
	uint32_t loopback_mode, reg_1, reg_2, sinegen_timing;

	tr_dbg(&sgen_tr, "sgen_id %d, enable %d", sgen_id, enable);

	sinegen_timing = mt8196_sinegen_timing(rate);

	if (enable == 1) {
		/* set loopback mode */
		switch (sgen_id) {
		case MT8196_MEMIF_UL0:
			loopback_mode = MT8196_SGEN_UL0;
			break;
		case MT8196_MEMIF_UL1:
			loopback_mode = MT8196_SGEN_UL1;
			break;			
		case MT8196_MEMIF_UL2:
			loopback_mode = MT8196_SGEN_UL2;
			break;
//		case MT8196_MEMIF_UL26:
//			loopback_mode = MT8196_SGEN_UL26;
//			break;
		case MT8196_MEMIF_DL1:
			loopback_mode = MT8196_SGEN_DL1;
			break;
		case MT8196_MEMIF_DL_24CH:
			loopback_mode = MT8196_SGEN_DL_24CH;
			break;
		default:
...
{
	mt8196_afe_sinegen_enable(TEST_SGEN_ID, 48000, 1);
}

void afe_sinegen_disable(void)
{
	mt8196_afe_sinegen_enable(TEST_SGEN_ID, 48000, 0);
}
