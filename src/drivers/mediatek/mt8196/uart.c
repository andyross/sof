...
static void __uart_baudrate_config(unsigned int addr, unsigned int baudrate, unsigned int uartclk)
{
	...
	if (baudrate <= 115200) {
		...
		if ((baudrate == 500000) || (baudrate == 576000))
			baudrate = 460800;
		quot = SOF_DIV_ROUND_UP(uartclk, 4 * baudrate);
	} else {
		highspeed = 3;
		quot = SOF_DIV_ROUND_UP(uartclk, 256 * baudrate);
	}
...
void InitDebugSerial(void)
{
	...
	/* CLR AP uart1 clk CG */
	io_reg_write(0x16640028, io_reg_read(0x16640010) & 0x2);
	/* GPIO 162/163 mode select, mode 1 is AP UART */
	io_reg_write(0x1002d440, (io_reg_read(0x1002d440) & 0xFFFF00FF) | 0x1100);
