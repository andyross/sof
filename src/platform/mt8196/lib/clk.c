...
static void set_mux_adsp_sel(uint32_t value)
{
	...
	tr_info(&clkdrv_tr, "adsp_clk_mux=%x, CLK_CFG_17=0x%08x\n",
...
static void set_mux_adsp_bus_sel(uint32_t value)
{
	...
	tr_info(&clkdrv_tr, "audio_local_bus_mux=%x, CLK_CFG_17=0x%08x\n",
	       value, io_reg_read(MTK_CLK_CFG_17));
}
...
{
	int i;
	tr_info(&clkdrv_tr, "enter\n");
	io_reg_write(0x1a000090, 0x127);
	sof->clocks = platform_shared_get(platform_clocks_info, sizeof(platform_clocks_info));
	io_reg_write(0x1a000090, 0x129);
	...
	io_reg_write(0x1a000090, 0x141);
	//clock_set_freq(CLK_CPU(cpu_get_id()), CLK_MAX_CPU_HZ);
	io_reg_write(0x1a000090, 0x143);
}
