...
SOF_DEFINE_REG_UUID(mtk_dai);

DECLARE_TR_CTX(mtk_dai_tr, SOF_UUID(mtk_dai_uuid), LOG_LEVEL_INFO);
...
static int afe_dai_handshake[MT8196_DAI_NUM] = {
	...
	//AFE_HANDSHAKE(MT8196_DAI_I2S_IN6, MT8196_IRQ_0, MT8196_MEMIF_UL26),
};
...
int dai_init(struct sof *sof)
{
	...
	for (i = 0; i < ARRAY_SIZE(afe_dai); i++) {
		...
		tr_info(&mtk_dai_tr, "afe_dai[%d].index : %d\n", i, afe_dai[i].index);
		...
		tr_info(&mtk_dai_tr, "afe_dai_handshake[%d] : 0x%x\n", i, afe_dai_handshake[i]);
	}
